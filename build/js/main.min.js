...
alert(1));var x=new THREE.Scene,m=new THREE.WebGLRenderer({antialias:!0}),ca=document.getElementById("container"),F,N,k,C,I=vec2.fromValues(0,0),h=1/15*10,f=1/15*6,q=vec3.fromValues(h,f,1),r=.05*f,D=vec2.fromValues(.2*f,.2*f),d,O,l,B=[];(function(){P();var a=THREE.ImageUtils.loadTexture("images/bw.png");a.repeat.set(4,2);a.wrapS=a.wrapT=THREE.RepeatWrapping;a.anisotropy=16;var b=THREE.SmoothShading;O=[];O.push(new THREE.MeshPhongMaterial({map:a,bumpMap:a,bumpScale:1,color:16777215,ambient:15658734,
specular:14540253,shininess:50,shading:b}));a=new THREE.MeshBasicMaterial({});a=O[0];k=new THREE.Mesh(new THREE.SphereGeometry(1,32,32),a);k.scale.multiplyScalar(r);x.add(k);a=new THREE.SpriteMaterial({map:new THREE.ImageUtils.loadTexture("images/glow.png"),useScreenCoordinates:!1,color:7829367,transparent:!1,blending:THREE.AdditiveBlending});a=new THREE.Sprite(a);a.scale.set(4*r,4*r,4*r);k.add(a);a=new THREE.MeshPhongMaterial({color:6710886});l=[];var b=new THREE.PlaneGeometry(1,1),c=new THREE.MeshBasicMaterial({color:2289390,
...
N=new THREE.PerspectiveCamera(55,h/f,f/2-.1,10);N.position.z=.5+f/2/Math.tan(27.5/180*Math.PI);a=new THREE.AmbientLight(8947848);x.add(a);(new THREE.DirectionalLight(16777215,2)).position.set(0,0,1);C=new THREE.PointLight(13421772,.7,100);C.position.set(0,0,0);x.add(C);F=new Stats;F.domElement.style.position="absolute";F.domElement.style.top="0px";ca.appendChild(m.domElement);ca.appendChild(F.domElement);window.addEventListener("resize",P,!1);m.domElement.addEventListener("mousemove",da,!1);m.domElement.addEventListener("touchstart",
R,!1);m.domElement.addEventListener("touchmove",R,!1)})();ba();var y=1/60;setTimeout(S,y);(function(a){a.requestFullscreen?a.requestFullscreen():a.mozRequestFullScreen?a.mozRequestFullScreen():a.webkitRequestFullscreen?a.webkitRequestFullscreen():a.msRequestFullscreen&&a.msRequestFullscreen()})(document.body);var H;B[0]=[];new THREE.Vector3(1/6,1/7,.125);var c,g=vec3.fromValues(0,0,0),w=vec3.fromValues(0,0,0),p,v,K=2,la=.2,A=0,T=Date.now(),G=1,J=2,L=3,u=G,E=vec3.fromValues(0,0,-q[2]/2),z=vec3.fromValues(0,
0,q[2]/2),t=vec4.fromValues(),ha=30,ea=1,fa=.03,ga=.001,ia=.1,W=.6,ja=.6,ka=3,Z=.2,Y=[],X=[],M;$("canvas").click(function(){switch(u){case G:console.log("start");v=.7;M=!0;p=vec4.fromValues(1,0,0,0);t=quat.fromValues();quat.identity(t);c=[];vec3.subtract(c,z,g);n(c,v/vec3.length(c));u=J;break;case L:u=G}});c=[1,1,1,1]});
